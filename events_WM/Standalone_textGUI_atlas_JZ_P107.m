...
%% which files to sort
paths=[];
paths.patientID='P107CS'; % label in plots
paths.basePath=['\\smb.researchcampus.csmc.edu\rutishauserulab-data\dataRawEpilepsy\P107CS\20250220_WMbinding_JZ\']; %add getuserdir in brackets if files are in C: ex. [getuserdir'\Documents\Practice OSort\P39CS_080515'];

filesToProcess = [209:216];  %which channels to detect/sort
...
groundChannels = [216]; %which channels are ground (they are ignored)

doGroundNormalization = 1;
normalizeOnly = [209:215]; %which channels to use for normalization (common average subtraction)
