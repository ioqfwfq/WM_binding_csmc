...
starting_text = '¿Listo? Presiona el botón central para comenzar el experimento.';
...
for i_trial = startintrial:c.nTrials
    ...
    if i_trial == c.nTrials || i_trial == c.nTrials/2
        ...
        if i_trial~= c.nTrials
            ...
            feedback = sprintf('¡DESCANSO! Respuestas correctas: %.02f%%', acc);
        else
            sendTTL(c.marker.expend)
            sprintf('¡Gracias! Respuestas correctas: %.02f%%', acc);
        end
